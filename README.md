...
| [double-free](double-free/) | CWE-415 — Double Free | The temporal sibling Revenant deliberately avoided (codename **Encore**) — 8 variants from a plain `free`-twice to alias/error-path/ownership-transfer/shared-member/`realloc` shapes and a reuse-aliasing bridge that turns the double-free into a controlled indirect call. The victim is the **allocator**, not your data; unlike UAF there *is* runtime detection (debug CRT, segment-heap fast-fail, Page Heap). Capstone: **Encore** (forthcoming) |

**Planned exhibits** (each with its own Axis A/B grid and capstone codename): integer overflow → undersized allocation (**Capsize**) · signedness bug (**Polarity**) · out-of-bounds read / info leak (**Trespass**) · uninitialized memory use (**Residue**). These are the corruption-class bugs with sharp, teachable binary signatures. (Data-flow-to-sink bugs — command injection, SQLi, path traversal — compile to unremarkable asm and belong in a separate wing, if at all.)
